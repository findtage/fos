...
// Determine client path based on whether we're running from dist or source
const isDev = __dirname.includes('server') && !__dirname.includes('dist');
const clientPath = isDev 
  ? path.join(__dirname, '../client')  // For ts-node dev mode (server/server.ts)
  : path.join(__dirname, '../../client'); // For compiled JS (dist/server/server.js)
...
// Handle SPA routes - serve index.html for any other routes
app.get("*", (req, res) => {
    res.sendFile(path.join(clientPath, 'index.html'));
});
